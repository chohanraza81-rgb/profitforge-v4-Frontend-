...
export default function CopyAllData({ data }) {
  ...
  const generatePlain = (d) => {
    ...
    for (const [key, score] of Object.entries(d.scores)) {
      if (key === 'confidence') continue;
      txt += `${key}: ${score.value.toFixed(1)}/10 (${score.source})\n`;
    }
    return txt;
  };
  ...
  const handleCopy = () => {
    ...
    if (format === 'markdown') text = generateMarkdown(data);
      ...
      for (const [key, score] of Object.entries(data.scores)) {
        if (key === 'confidence') continue;
        csv += `${key},${score.value.toFixed(1)},${score.source}\n`;
      }
      text = csv;
    } else if (format === 'json') {
  ...
  return (
    <div className="glass p-4 flex items-center gap-4">
      <select value={format} onChange={e => setFormat(e.target.value)} className="bg-transparent border border-white/10 rounded px-2 py-1">
        <option value="markdown">Markdown</option>
        <option value="plain">Plain Text</option>
        <option value="csv">CSV</option>
        <option value="json">JSON</option>
      </select>
      <button onClick={handleCopy} className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition">
        <Copy size={16} /> Copy All Data
      </button>
    </div>
  );
}
